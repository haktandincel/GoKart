...
public class GameFlowManager : MonoBehaviour
{
    ...
    public ArcadeKart player2Kart;

    ArcadeKart[] karts;
    ObjectiveManager m_ObjectiveManager;
    TimeManager m_TimeManager;
    SplitScreenCameraManager m_SplitScreenManager;
    ...
    void Start()
    {
        if (autoFindKarts)
        {
            ...
            if (karts.Length > 0)
            {
                ...
                if (twoPlayerMode && karts.Length > 1)
                {
                    player2Kart = karts[1];
                }
            }
        ...
        // Setup split-screen if two-player mode
        if (twoPlayerMode && playerKart != null && player2Kart != null)
        {
            m_SplitScreenManager = FindObjectOfType<SplitScreenCameraManager>();
            if (m_SplitScreenManager == null)
            {
                Debug.LogWarning("SplitScreenCameraManager not found. Creating one...");
                GameObject splitScreenObj = new GameObject("SplitScreenCameraManager");
                m_SplitScreenManager = splitScreenObj.AddComponent<SplitScreenCameraManager>();
            }

            if (playerKart.vCam != null)
                m_SplitScreenManager.SetPlayer1Target(playerKart.transform);
            if (player2Kart.vCam != null)
                m_SplitScreenManager.SetPlayer2Target(player2Kart.transform);
        }
